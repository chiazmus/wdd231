...
function addCourse(course){
    let newCourse = document.createElement('h2');
...
function creditSum(courses){
    let credits = courses.reduce((accumulator, item) => accumulator + item.credits, 0);
    let creditElement = document.createElement('p');
    creditElement.textContent = `Total credits: ${credits}`;
    courseDiv.appendChild(creditElement);    
}

wddButton.addEventListener('click', () => {
    clearCourses();
    let wddCourses = courses.filter(course => course.subject === 'WDD');
    creditSum(wddCourses);
    wddCourses.forEach(addCourse);
});

cseButton.addEventListener('click', () => {
    clearCourses();
    let cseCourses = courses.filter(course => course.subject === 'CSE');
    creditSum(cseCourses);
    cseCourses.forEach(addCourse);
});
...
//Gotta make sure you start off with some courses to begin with...
creditSum(courses);
courses.forEach(addCourse);
